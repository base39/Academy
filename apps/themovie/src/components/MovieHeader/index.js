  ...
  useEffect(() => {
    const fetchMovie = async () => {
      await fetch(`http://localhost:8080/movies/${id}?language=pt-BR`)
        ...
        .then(result => {
          setMovie(result);
          setMovieHours(moment.duration(result?.runtime, 'minutes').get('hours'));
          setMovieMinutes(moment.duration(result?.runtime, 'minutes').get('minutes'));
        });
    }
    fetchMovie()
  }, [id]);
        ...
        <Container>
          <HeaderDetails>
            <ImageWrapper item md={4} noWrap>
              <Image src={`//image.tmdb.org/t/p/w300_and_h450_bestv2/${movie?.poster_path}`} alt={movie?.poster_path} />
            </ImageWrapper>
            <DetailsWrapper item md={8} noWrap>
              <div>
                <MovieTitle variant="h4">
                  {movie?.title} <MovieYear>({moment(movie?.release_date).year()})</MovieYear>
                </MovieTitle>
                <MovieRelease>{moment(movie?.release_date).format("DD/MM/YYYY")}</MovieRelease>
